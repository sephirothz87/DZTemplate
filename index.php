...
fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "index.php start" . "\n" );

fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "SERVER['QUERY_STRING'] = ".$_SERVER['QUERY_STRING'] . "\n" );
...
if(!empty($_SERVER['QUERY_STRING']) && is_numeric($_SERVER['QUERY_STRING'])) {
	...
	fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "SERVER['QUERY_STRING'] null" . "\n" );
	...
	//MEMO 此时DOMAIN=NULL
// 	if($domain){
// 		fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "domain true" . "\n" );
// 	}else{
// 		fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "domain false" . "\n" );
// 	}
// 	foreach ($domain as $key => $value) { fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s' ,time()+60*60*6). "]". "domain:" . $key . ": " .$value."\n" ); }

	@include_once './data/sysdata/cache_domain.php';
	$_ENV['domain'] = $domain;

	fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "_ENV['domain'] = ".$_ENV['domain'] . "\n" );
	foreach ($domain as $key => $value) { fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s' ,time()+60*60*6). "]". "domain:" . $key . ": " .$value."\n" ); }
	
	if(empty($_ENV['domain'])) {
		$_ENV['curapp'] = 'forum';
		fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "curapp = FORUM" . "\n" );
	} else {
		...
		if(!empty($_ENV['domain']['app']) && is_array($_ENV['domain']['app']) && in_array($_SERVER['HTTP_HOST'], $_ENV['domain']['app'])) {
			fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113155" . "\n" );
			...
			fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113211" . "\n" );
			...
			fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113236" . "\n" );
			$jump = true;
		}
		if(empty($url) && empty($_ENV['curapp'])) {
			fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113351" . "\n" );
			if(!empty($_ENV['domain']['defaultindex']) && !$jump) {
				fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113436" . "\n" );
				...
				fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113445" . "\n" );
				if($jump) {
					fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113601" . "\n" );
					$url = empty($_ENV['domain']['app']['default']) ? (!empty($_ENV['domain']['defaultindex']) ? $_ENV['domain']['defaultindex'] : 'forum.php') : 'http://'.$_ENV['domain']['app']['default'];
					if(empty($_ENV['domain']['app']['default'])){
						fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113645" . "\n" );
					}else{
						fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113651" . "\n" );
					}
					if(!empty($_ENV['domain']['defaultindex'])){
						fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113928" . "\n" );
					}else{
						fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113931" . "\n" );
					}
					fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "url = ".$url . "\n" );
					
				} else {
					fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008113604" . "\n" );
					$_ENV['curapp'] = 'forum';
				}
			}
		}
...
if(!empty($url)) {
	fwrite(fopen( "D:\\tmp\\log.txt" , "a" ), "[" .date('Y-m-d H:i:s',time()+60*60*6). "]". "20151008174403" . "\n" );
